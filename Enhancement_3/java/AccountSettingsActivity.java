...
public class AccountSettingsActivity extends AppCompatActivity {
    ...
    private void openResetPasswordDialog() {
        ...
        saveBtn.setOnClickListener(v -> {
            ...
            // PASSWORD VALIDATION
            if (!PasswordValidation.isValidPassword(newPass)) {
                Toast.makeText(this,
            ...
            // Hash current password to compare to stored value
            String hashedCurrent = PasswordHash.hashPassword(current);

            // Checks current password to be sure this is correct before allowing a reset
            if (!storedPassword.equals(hashedCurrent)) {
                Toast.makeText(this, "Current password is incorrect", Toast.LENGTH_SHORT).show();
                return;
            }

            // Hash new password before saving
            String hashedNewPassword = PasswordHash.hashPassword(newPass);

            // Updates new password to be the current password
            dbHelper.updatePassword(loggedInUsername, hashedNewPassword);
